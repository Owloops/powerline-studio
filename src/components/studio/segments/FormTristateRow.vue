...
import type { SegmentKey, SegmentTristateKey } from './segmentMeta'

/**
 * Tri-state select for a segment flag whose unset state is meaningful: the CLI
 * falls back to another setting. Deliberately outside the formwerk form, which
 * writes every value through deepMerge and so can never express "no value".
 */
const props = defineProps<{
	segmentName: SegmentKey
	optionKey: SegmentTristateKey
	label: string
	defaultLabel: string
	disabled?: boolean
...
const currentValue = computed<string>(() => {
	const seg = configStore.currentLineSegments[props.segmentName] as unknown as
		| Record<string, unknown>
		| undefined
	const value = seg?.[props.optionKey]
...
function onValueChange(next: string) {
	...
	configStore.setSegmentTristate(
		editorStore.activeLineIndex,
		props.segmentName,
		props.optionKey,
		value,
	)
...
const fieldId = `${props.optionKey}-${props.segmentName}`
</script>
